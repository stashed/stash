...
import (
	apis "github.com/appscode/stash/apis"
...
type ResticLister interface {
	...
	List(selector labels.Selector) (ret []*apis.Restic, err error)
...
// List lists all Restics in the indexer.
func (s *resticLister) List(selector labels.Selector) (ret []*apis.Restic, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*apis.Restic))
	})
	return ret, err
}
...
type ResticNamespaceLister interface {
	...
	Get(name string) (*apis.Restic, error)
...
// List lists all Restics in the indexer for a given namespace.
func (s resticNamespaceLister) List(selector labels.Selector) (ret []*apis.Restic, err error) {
...
// Get retrieves the Restic from the indexer for a given namespace and name.
func (s resticNamespaceLister) Get(name string) (*apis.Restic, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(apis.Resource("restic"), name)
	}
	return obj.(*apis.Restic), nil
}
