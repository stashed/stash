...
When a snapshot is taken an event will be created. Event name will be `<name_of_tpr>-<backup_count>`. If a backup precess failed event reason will show us `Failed` else event reason will be `Success`
...
This will be a kloader type process that watches restic TPR and translates that into the restic compatiable config. eg,
