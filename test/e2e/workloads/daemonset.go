	...
	Context("DaemonSet", func() {
		...
		Context("Container inject/removal check", func() {
			It("should remove sidecar/init-container when the respective invoker is removed", func() {
				...
				f.EventuallyDaemonSet(dmn.ObjectMeta).ShouldNot(HaveInitContainer(apis.StashInitContainer))
				...
				f.EventuallyDaemonSet(dmn.ObjectMeta).ShouldNot(HaveSidecar(apis.StashContainer))
			})
		})
	})
})
