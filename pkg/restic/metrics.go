...
func (metricOpt *MetricsOptions) SendBackupSessionMetrics(config *rest.Config, namespace string, invokerInfo apis.InvokerInfo, targetInfo apis.TargetInfo, status api_v1beta1.BackupSessionStatus) error {
	// create metric registry
	registry := prometheus.NewRegistry()

	// generate metrics labels
	labels, err := backupMetricLabels(config, namespace, invokerInfo, targetInfo, metricOpt.Labels)
	if err != nil {
		return err
	}
	// create metrics
...
func (metricOpt *MetricsOptions) SendBackupHostMetrics(config *rest.Config, namespace string, invokerInfo apis.InvokerInfo, targetInfo apis.TargetInfo, backupOutput *BackupOutput) error {
	if backupOutput == nil {
	...
	if backupOutput.RepositoryStats.Integrity != nil {
		repoMetricLabels, err := repoMetricLabels(config, namespace, invokerInfo, targetInfo, metricOpt.Labels)
		if err != nil {
			return err
		}
...
func backupMetricLabels(config *rest.Config, namespace string, invokerInfo apis.InvokerInfo, targetInfo apis.TargetInfo, userProvidedLabels []string) (prometheus.Labels, error) {
	...
	if invokerInfo.Driver == api_v1beta1.VolumeSnapshotter {
		...
		if targetInfo.Target != nil {
			labels, err := targetLabels(config, targetInfo.Target.Ref, namespace)
			if err != nil {
				return nil, err
			}
			promLabels = upsertLabel(promLabels, labels)
		}
		promLabels[MetricsLabelRepository] = invokerInfo.RepoName
	}
...
func repoMetricLabels(clientConfig *rest.Config, namespace string, invokerInfo apis.InvokerInfo, targetInfo apis.TargetInfo, userProvidedLabels []string) (prometheus.Labels, error) {
	// add user provided labels
	promLabels := parseUserProvidedLabels(userProvidedLabels)
	...
	if targetInfo.Target != nil {
		...
		repository, err := stashClient.StashV1alpha1().Repositories(namespace).Get(invokerInfo.RepoName, metav1.GetOptions{})
		if err != nil {
			return nil, err
		}
