...
type RecoveryLister interface {
	...
	List(selector labels.Selector) (ret []*stash.Recovery, err error)
...
// List lists all Recoveries in the indexer.
func (s *recoveryLister) List(selector labels.Selector) (ret []*stash.Recovery, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*stash.Recovery))
	})
	return ret, err
}
...
type RecoveryNamespaceLister interface {
	...
	Get(name string) (*stash.Recovery, error)
...
// List lists all Recoveries in the indexer for a given namespace.
func (s recoveryNamespaceLister) List(selector labels.Selector) (ret []*stash.Recovery, err error) {
...
// Get retrieves the Recovery from the indexer for a given namespace and name.
func (s recoveryNamespaceLister) Get(name string) (*stash.Recovery, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(stash.Resource("recovery"), name)
	}
	return obj.(*stash.Recovery), nil
}
