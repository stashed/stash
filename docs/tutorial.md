...
Here,
 ...
 - `spec.fileGroups` indicates an array of local paths that will be backed up using restic. For each path, users can also define the retention policy for old snapshots. Here, we are backing up the `/lib` folder and only keeping the last 5 snapshots.
