		...
		cache.ResourceEventHandlerFuncs{
			...
			UpdateFunc: func(old, new interface{}) {
				...
				if oldObj.ObjectMeta.Annotations != nil {
					oldImage = oldObj.ObjectMeta.Annotations[VersionTag]
				}
				if newObj.ObjectMeta.Annotations != nil {
					newImage = newObj.ObjectMeta.Annotations[VersionTag]
				}
...
func (c *Controller) updateObjectAndStartBackup(r *sapi.Restic) error {
	ls := labels.SelectorFromSet(labels.Set{ConfigName: r.Name})
	ob, typ, err := getKubeObject(c.KubeClient, r.Namespace, ls)
	if err != nil {
		return err
	}
	if ob == nil || typ == "" {
	...
	case ReplicationController:
		...
		newRC, err := c.KubeClient.CoreV1().ReplicationControllers(r.Namespace).Update(rc)
		if err != nil {
			return err
		}
		opts.LabelSelector = findSelectors(newRC.Spec.Selector).String()
		if err = c.restartPods(c.KubeClient, r.Namespace, opts); err != nil {
			return err
		}
	case ReplicaSet:
		...
		newReplicaset, err := c.KubeClient.ExtensionsV1beta1().ReplicaSets(r.Namespace).Update(replicaset)
		if err != nil {
			return err
		}
		opts.LabelSelector = findSelectors(newReplicaset.Spec.Template.Labels).String()
		if err = restartPods(c.KubeClient, r.Namespace, opts); err != nil {
			return err
		}
	case Deployment:
		...
		_, err = c.KubeClient.ExtensionsV1beta1().Deployments(r.Namespace).Update(deployment)
		if err != nil {
			return err
		}
	case DaemonSet:
		...
		newDaemonset, err := c.KubeClient.ExtensionsV1beta1().DaemonSets(r.Namespace).Update(daemonset)
		if err != nil {
			return err
		}
...
func (c *Controller) updateObjectAndStopBackup(r *sapi.Restic) error {
	ls := labels.SelectorFromSet(labels.Set{ConfigName: r.Name})
	ob, typ, err := getKubeObject(c.KubeClient, r.Namespace, ls)
	if err != nil {
		return err
	...
	case ReplicationController:
		...
		return restartPods(c.KubeClient, r.Namespace, opts)
	case ReplicaSet:
		replicaset := &extensions.ReplicaSet{}
		if err := yaml.Unmarshal(ob, replicaset); err != nil {
			return err
...
func (c *Controller) updateImage(r *sapi.Restic, image string) error {
	ls := labels.SelectorFromSet(labels.Set{ConfigName: r.Name})
	ob, typ, err := getKubeObject(c.KubeClient, r.Namespace, ls)
	if err != nil {
		return err
