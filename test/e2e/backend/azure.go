...
var _ = Describe("Azure Backend", func() {
	...
	var (
		setupAzureRepository = func() (*api.Repository, error) {
			...
			repo := f.NewAzureRepository(cred.Name, 0)
	...
	Context("General Backup/Restore", func() {
		It("should backup/restore in/from Azure backend", func() {
			// Deploy a Deployment
			...
			// Setup a Azure Repository
			repo, err := f.SetupAzureRepository(0)
			Expect(err).NotTo(HaveOccurred())
	...
	Context("Backup/Restore with maxConnection", func() {
		It("should backup/restore with maxConnection", func() {
			...
			// Setup a Azure Repository
			repo, err := f.SetupAzureRepository(9)
			Expect(err).NotTo(HaveOccurred())
