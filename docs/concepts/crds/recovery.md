...
spec:
  ...
  snapshot: deployment.stash-demo-e0e9c272 # ignore this field to recover latest snapshot
  paths:
  - /source/data
  recoveredVolumes:
...
Indicates the name of the `Repository` CRD that represents respective **restic** repository where the backed up snapshots are stored. To know more about `Repository` CRD see [here](/docs/concepts/crds/repository.md).

### spec.snapshot

Indicates the name of the `Snapshot` object that represents **restic** backup snapshot. **This field enables the users to recover specific snapshot**. To recover the latest snapshot, just do not use this field. To know more about `Snapshot` see [here](/docs/concepts/crds/snapshot.md).
...
Note that, `Recovery` recovers data in the same paths from where the backup was taken (specified in `spec.paths`). So, volumes must be mounted on those paths or their parent paths.
